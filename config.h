...
/*SchemeNorm      普通      */   {0,    0xff, borderalpha},
/*SchemeSel       选中      */
                                 {0,    0xff, borderalpha},
...
//-----------------------------------------------------------------------------
//                     状态栏启动脚本
static const char *statusbarscript = "~/.dwm/statusbar-rs/target/release/statusbar-rs";
...
/* spawn + SHCMD 执行对应命令(已下部分建议完全自己重新定义) */
        ...
        {Mod1Mask, XK_space, spawn,
         SHCMD("~/.config/rofi/launchers/type-3/launcher.sh")},            /* alt space        | rofi: 执行drun         */
        ...
        {MODKEY, XK_v, spawn,
         SHCMD("$DWM/.bin/clip_history.sh")},                              /* super v          | 剪切板历史             */
        {MODKEY | ShiftMask, XK_a, spawn,
         SHCMD("flameshot gui -c -p ~/Pictures/screenshots")},             /* super shift a    | 截图                   */
        ...
        {MODKEY, XK_c, spawn,
         SHCMD("google-chrome-stable --force-device-scale-factor=1.25")},  /* super c    | chrome               */
        // { MODKEY,              XK_i,      spawn, SHCMD("/home/gb/opt/idea-IU-213.5744.223/bin/idea.sh") },          /* super i    | idea               */
        {MODKEY, XK_F1, spawn,
         SHCMD("killall pcmanfm || pcmanfm")},                             /* super F1         | 打开/关闭pcmanfm       */
        {MODKEY, XK_p, spawn,
         SHCMD("$DWM/.bin/rofi.sh")},                                       /* super p          | rofi: 执行自定义脚本   */
        {MODKEY, XK_n, spawn,
         SHCMD("i3lock")},                                   /* super n          | 锁定屏幕               */
        {MODKEY | ShiftMask, XK_Up, spawn,
         SHCMD("$DWM/.bin/set_vol.sh up")},                                 /* super shift up   | 音量加                 */
        {MODKEY | ShiftMask, XK_Down, spawn,
         SHCMD("$DWM/.bin/set_vol.sh down")},                               /* super shift down | 音量减                 */
//-----------------------------------------------------------------------------
...
//=============================================================================
        ...
        {0, XF86XK_MonBrightnessUp, spawn,
         SHCMD("light -A 5; notify-send -r 9123 -h int:value:`light` -h string:hlcolor:#dddddd 'Backlight' ")},
        {0, XF86XK_MonBrightnessDown, spawn,
         SHCMD("light -U 5; notify-send -r 9123 -h int:value:`light` -h string:hlcolor:#dddddd 'Backlight' ")},
